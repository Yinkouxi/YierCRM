  ...
  <el-container>
    <el-main>
      ...
      <el-card shadow="never">
        <div class="toolbar">
          <el-button type="primary" icon="Plus" style="margin-right: 5px" @click="add"
            >新建咨询</el-button
          >
          <el-button type="warning" icon="Connection" @click="transfer">移交客户</el-button>
        </div>
    ...
    <transferDialog
      ...
      v-model:transferVisible="transferVisible"
      :ids="ids"
      :names="names"
      @transferChange="getConsultPage"
    ></transferDialog>
  </el-container>
</template>
...
import transferDialog from './myClient/transferDialog.vue'
...
const selectionTable = (selection: IConsultList[]) => {
  ids.value = selection.map((item) => item.id)
  names.value = selection.map((item) => item.name)
}

//移交客户
let transferVisible = ref<boolean>(false)
const transfer = () => {
